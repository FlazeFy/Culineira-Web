    ...
    <li class="card card-blog mb-4">
        ...
        <div class="table">
            ...
            @if(($gl->context2 == "Public")&&($i == 0))
                <form method="POST" action="/global/join/{{$gl->id}}">
                    @csrf
            ...
            @elseif (($gl->context2 == "Private")&&($i == 0))
                <form method="POST" action="/global/request/{{$gl->id}}">
                    @csrf
                    <input name="groupname" value="{{$gl->name}}" hidden>
                ...
                <div class="col-md-6">
                    ...
                    @foreach($following as $fsw)
                        @if($fsw->user_id_2 == $gl->id)
                            <form action="/global/unfollow/{{$fsw->id}}" method="POST">
                                @csrf
                    ...
                    @endforeach
                    @if($i == 0)
                        <form action="/global/follow/{{$gl->id}}" method="POST">
                            @csrf
                            <input hidden name="username" value="{{$gl->name}}">
